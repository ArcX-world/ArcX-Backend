 ...
 * 抓娃娃下爪定时器
 */
public class CatchDollOffLineTask extends ScheduledTask {
    ...
    public CatchDollOffLineTask(int userId, int productId, int time, int gamingState, long onProductTime) {
        super("抓娃娃定时下机");
        this.userId = userId;
    ...
    public void run() {
        ...
        LogUtil.getLogger().info("玩家{}在设备{}上抓娃娃定时下机---------",userId,productId);
        //获取设备锁
            ...
            if (lock.lock()) {
                ...
                if(roomMsg!=null){
                    ...
                    if(roomMsg.getGamingUserId()==userId && ProductUtil.startGameTime(productId) ==time
                        ...
                        LogUtil.getLogger().info("玩家{}在设备{}上抓娃娃成功定时下机---------",userId,productId);
                        //设备操作
                        ProductSocketOperateService.catchDollOperate(productId, ProductOperationEnum.OFF_LINE.getCode(),userId);
                    }
                }
            }
        }catch (Exception e){
        ...
        //下机失败，重新处理，继续启用定时器
        if(flag){
            LogUtil.getLogger().info("玩家{}在娃娃机设备{}上下机失败，重新处理--------", userId, productId);
            SchedulerSample.delayed(100, new
                    CatchDollOffLineTask(userId,productId,time,gamingState, onProductTime));
        }
    }
}
