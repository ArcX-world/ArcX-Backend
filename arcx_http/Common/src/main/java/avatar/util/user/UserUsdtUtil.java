        ...
        try {
            if (lock.lock()) {
                if(usdtBalance(userId)>=costNum) {
                    ...
                    UserUsdtBalanceEntity entity = UserUsdtBalanceDao.getInstance().loadByMsg(userId);
                    if (entity != null) {
                        double usdtNum = entity.getNum();//玩家商品数量
                        entity.setNum(Math.max(0, (StrUtil.truncateFourDecimal(usdtNum-costNum))));//商品数量
                        flag = UserUsdtBalanceDao.getInstance().update(entity);
                    } else {
                        LogUtil.getLogger().error("扣除玩家{}USDT数量{}的时候，查询不到玩家平台余额信息-----", userId, costNum);
                    }
                }
            }
        }catch (Exception e){
